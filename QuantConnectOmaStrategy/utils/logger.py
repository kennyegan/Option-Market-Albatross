...
class StrategyLogger:
    ...
    def log(self, 
        ...
        if self.enable_file_logging and self.output_mode in [LogOutputMode.FILE, LogOutputMode.BOTH]:
    ...
    def log_trade(self, 
        ...
        trade_entry = {
            ...
            'quantity': quantity,
            'price': price,
            'value': abs(quantity * price * 100) if symbol.SecurityType == SecurityType.Option else abs(quantity * price),
            'reason': reason,
    ...
    def log_risk_snapshot(self, snapshot_data: Dict) -> None:
        ...
        if self.last_risk_snapshot_time:
            elapsed = (self.algorithm.Time - self.last_risk_snapshot_time).total_seconds() / 60
    ...
    def log_risk_metrics(self, risk_metrics: Dict) -> None:
        ...
        if risk_metrics.get('event'):
            ...
            if 'VEGA' in str(risk_metrics.get('event', '')).upper() or \
               'DELTA' in str(risk_metrics.get('event', '')).upper() or \
               'GAMMA' in str(risk_metrics.get('event', '')).upper():
    ...
    def log_performance(self, checkpoint: str = "default") -> None:
        ...
        metrics = {
            ...
            'return_pct': (current_value / self.starting_capital - 1) if self.starting_capital > 0 else 0
        }
    ...
    def log_daily_summary(self, 
        ...
        daily.daily_return_pct = daily.daily_pnl / daily.starting_nav if daily.starting_nav > 0 else 0
        ...
        if tca_summary:
            ...
            daily.net_execution_cost_bps = daily.avg_slippage_bps - daily.avg_spread_capture_bps
    ...
    def _check_day_rollover(self) -> None:
        ...
        if self.current_date != current:
            ...
            self.current_daily = DailySummary(
                date=current,
                starting_nav=self.algorithm.Portfolio.TotalPortfolioValue
            )
    ...
    def get_alpha_diagnostics_summary(self) -> Dict:
        ...
        iv_rv_ratios = [d.iv_rv_ratio for d in self.alpha_diagnostics if d.iv_rv_ratio > 0]
