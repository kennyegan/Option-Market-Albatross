...
class RiskConfig:
    ...
    regime_delta_multiplier: Dict[str, float] = field(default_factory=lambda: {
        'CALM': 1.2,
        'NORMAL': 1.0,
        'STRESSED': 0.7,
        'CRISIS': 0.3
    })
    regime_vega_multiplier: Dict[str, float] = field(default_factory=lambda: {
        'CALM': 1.2,
        'NORMAL': 1.0,
        'STRESSED': 0.5,
        'CRISIS': 0.2
...
class ExposureLimitsRiskManagementModel(RiskManagementModel):
    ...
    def _update_daily_tracking(self, algorithm: QCAlgorithm) -> None:
        ...
        if self.last_reset_date != current_date:
            ...
            if self.logger:
                self.logger.log(
                    f"Daily reset | Starting NAV: ${current_value:,.2f}",
                    LogLevel.INFO
                )
        ...
        if self.daily_starting_value:
            ...
            self.risk_metrics['daily_pnl_pct'] = self.daily_pnl / self.daily_starting_value
            
            # Track high water for intraday drawdown
            if current_value > self.daily_high_water:
                self.daily_high_water = current_value
            
            intraday_drawdown = (self.daily_high_water - current_value) / self.daily_high_water
            self.risk_metrics['max_drawdown'] = max(self.risk_metrics['max_drawdown'], intraday_drawdown)
    ...
    def _check_daily_loss_limit(self, algorithm: QCAlgorithm) -> bool:
        ...
        current_value = algorithm.Portfolio.TotalPortfolioValue
        pnl_pct = (current_value - self.daily_starting_value) / self.daily_starting_value
    ...
    def _run_scenario_analysis(self, algorithm: QCAlgorithm) -> List[ScenarioResult]:
        ...
        for scenario in self.config.scenarios:
            ...
            for holding in algorithm.Portfolio.Values:
                if holding.Symbol.SecurityType in [SecurityType.Equity, SecurityType.Index]:
    ...
    def _check_scenario_limits(self, 
                               algorithm: QCAlgorithm,
                               scenario_results: List[ScenarioResult]) -> List[PortfolioTarget]:
        """
    ...
    def _check_greek_limits(self, 
                            algorithm: QCAlgorithm,
                            regime: str) -> List[PortfolioTarget]:
        """
    ...
    def _reduce_vega_exposure(self, 
                              algorithm: QCAlgorithm,
                              scale: float = 0.5) -> List[PortfolioTarget]:
        """
        ...
        for holding in algorithm.Portfolio.Values:
            if holding.Symbol.SecurityType == SecurityType.Option and holding.Quantity != 0:
                ...
                if greeks:
                    ...
                    vega_positions.append((holding.Symbol, holding.Quantity, vega_contrib))
    ...
    def _reduce_gamma_exposure(self,
                               algorithm: QCAlgorithm,
                               scale: float = 0.5) -> List[PortfolioTarget]:
        """
        ...
        for holding in algorithm.Portfolio.Values:
            if holding.Symbol.SecurityType == SecurityType.Option and holding.Quantity != 0:
                ...
                if greeks:
                    ...
                    gamma_positions.append((holding.Symbol, holding.Quantity, gamma_contrib))
    ...
    def _create_delta_hedge(self, algorithm: QCAlgorithm, current_delta: float) -> Optional[PortfolioTarget]:
    ...
    def _check_position_age(self, algorithm: QCAlgorithm) -> List[PortfolioTarget]:
        ...
        for holding in algorithm.Portfolio.Values:
            ...
            age_hours = (current_time - self.position_entry_times[symbol]).total_seconds() / 3600
            ...
            if age_hours > self.config.max_position_age_hours:
                ...
                self._log_risk_event(
                    ...
                    "POSITION_AGE",
                    "MaxPositionAge",
                    {'symbol': str(symbol), 'age_hours': age_hours, 'limit': self.config.max_position_age_hours},
        ...
        for holding in algorithm.Portfolio.Values:
            ...
            if position_pct > self.config.max_position_pct:
                ...
                self._log_risk_event(
                    ...
                    {'symbol': str(symbol), 'position_pct': position_pct, 'limit': self.config.max_position_pct},
                    RiskAction.CLOSE_POSITION,
                    [symbol]
                )
            ...
            # Loss check
            unrealized_loss_pct = -holding.UnrealizedProfitPercent if holding.UnrealizedProfitPercent < 0 else 0
            ...
            if unrealized_loss_pct > self.config.max_unrealized_loss_pct:
                ...
                self._log_risk_event(
                    ...
                    "UNREALIZED_LOSS",
                    "MaxUnrealizedLoss",
                    {'symbol': str(symbol), 'loss_pct': unrealized_loss_pct, 'limit': self.config.max_unrealized_loss_pct},
                    RiskAction.CLOSE_POSITION,
                    [symbol]
                )
    ...
    def _filter_new_risk(self, 
                         original_targets: List[PortfolioTarget],
                         risk_targets: List[PortfolioTarget]) -> List[PortfolioTarget]:
    ...
    def get_scenario_summary(self, algorithm: QCAlgorithm) -> Dict:
        ...
        return {
            ...
            'worst_scenario': min(results, key=lambda x: x.estimated_pnl).scenario.name if results else None,
