...
class VolatilityRegimeClassifier:
    ...
    def update(
        ...
        if (
            ...
            and self.rv_calculator is not None
            and underlying_symbol is not None
        ):
            realized_vol = self.rv_calculator.get_realized_vol(
                underlying_symbol, method="ensemble"
            )
    ...
    def _calculate_confidence(
        self, vix_score: float, rv_score: float, returns_score: float
    ) -> float:
        """
    ...
    def _get_vix_percentile(self, vix_value: Optional[float]) -> Optional[float]:
        ...
        percentile = (
            np.sum(np.array(historical_vix) < vix_value) / len(historical_vix)
        ) * 100
