...
class IVRVSpreadAlphaModel(AlphaModel):
    ...
    def Update(self, algorithm: QCAlgorithm, data: Slice) -> List[Insight]:
        ...
        if regime and self.config.disable_short_vol_in_crisis:
            if self.regime_classifier and self.regime_classifier.is_crisis():
                if self.logger:
                    self.logger.log("Alpha model disabled - CRISIS regime", LogLevel.WARNING)
                return insights
    ...
    def _get_rv_data(self, underlying_symbol: Symbol) -> Optional[Dict]:
        ...
        rv_data = {
            ...
            'ewma': summary.ewma,
            'high_uncertainty': summary.has_high_uncertainty(self.config.max_rv_uncertainty_ratio)
        }
    ...
    def _analyze_contract(self,
                         ...
                         rv_data: Dict,
                         regime: Optional[str],
                         algorithm: QCAlgorithm) -> Optional[Insight]:
        """
        ...
        if self.config.reduce_on_high_uncertainty and rv_data.get('high_uncertainty', False):
        ...
        iv_rv_signal = self._calculate_iv_rv_signal(
            contract, 
            rv_data['ensemble'],
            rv_data.get('high_uncertainty', False)
        )
    ...
    def _calculate_iv_rv_signal(self, 
                                contract,
                                rv: float,
                                high_uncertainty: bool) -> float:
        """
    ...
    def _create_insight(self,
        ...
        insight.Properties["IVRVRatio"] = iv / rv_data['ensemble'] if rv_data['ensemble'] > 0 else 0
        ...
        insight.Properties["SpreadPct"] = (contract.Ask - contract.Bid) / ((contract.Bid + contract.Ask) / 2) if contract.Bid + contract.Ask > 0 else 0
    ...
    def OnSecuritiesChanged(self, algorithm: QCAlgorithm, changes: SecurityChanges) -> None:
        ...
        for security in changes.RemovedSecurities:
            ...
            underlying = security.Symbol.Underlying if hasattr(security.Symbol, 'Underlying') else None
