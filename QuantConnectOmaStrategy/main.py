...
class OMAOptionsArbitrageAlgorithm(QCAlgorithm):
    ...
    def Initialize(self):
        ...
        self.logger.log(
            "Initializing OMA Options Arbitrage Strategy v2.0", LogLevel.INFO
        )
        self.logger.log("=" * 60, LogLevel.INFO)
        ...
        self.logger.log(
            f"RV Calculator initialized: lookback={self.rv_lookback}d, ewma_decay={self.ewma_decay}",
            LogLevel.INFO,
        )
        ...
        regime_config = RegimeConfig(
            ...
            vix_stressed_upper=float(
                self.GetParameter("vol-regime-vix-stressed") or 30
            ),
            ...
            rv_stressed_upper=float(
                self.GetParameter("vol-regime-rv-stressed") or 0.30
            ),
        )

        self.regime_classifier = VolatilityRegimeClassifier(
            self, rv_calculator=self.rv_calculator, config=regime_config
        )
        ...
        alpha_config = AlphaConfig(
            ...
            max_rv_uncertainty_ratio=float(
                self.GetParameter("max-rv-uncertainty-ratio") or 0.2
            ),
        ...
        self.logger.log(
            f"Alpha Model: IV/RV threshold={self.iv_rv_threshold}, moneyness=[{alpha_config.moneyness_min}-{alpha_config.moneyness_max}]",
            LogLevel.INFO,
        )
        ...
        self.logger.log(
            f"Portfolio Model: vega_limit={self.vega_limit}, delta_tol={self.delta_tolerance}, target_vol={self.target_daily_vol:.1%}",
            LogLevel.INFO,
        )
        ...
        execution_config = ExecutionConfig(
            ...
            limit_order_offset_bps=float(
                self.GetParameter("limit-order-offset-bps") or 10
            ),
            min_absolute_tick_offset=float(
                self.GetParameter("min-absolute-tick-offset") or 0.05
            ),
            min_edge_after_costs_bps=self.min_edge_bps,
            spread_cost_multiplier=float(
                self.GetParameter("spread-cost-multiplier") or 0.5
            ),
            impact_buffer_bps=float(self.GetParameter("impact-buffer-bps") or 5),
            max_participation_pct=float(
                self.GetParameter("max-participation-pct") or 0.20
            ),
            unfilled_timeout_seconds=int(
                self.GetParameter("unfilled-timeout-seconds") or 60
            ),
            max_replace_attempts=int(self.GetParameter("max-replace-attempts") or 3),
        )

        self.execution_model = SmartExecutionModel(
            logger=self.logger, config=execution_config
        )
        self.SetExecution(self.execution_model)
        self.logger.log(
            f"Execution Model: min_edge={self.min_edge_bps}bps, participation={execution_config.max_participation_pct:.0%}",
            LogLevel.INFO,
        )
        ...
        risk_config = RiskConfig(
            ...
            max_position_age_hours=int(
                self.GetParameter("max-position-age-hours") or 24
            ),
            max_position_pct=float(self.GetParameter("max-position-pct") or 0.10),
            max_unrealized_loss_pct=float(
                self.GetParameter("max-unrealized-loss-pct") or 0.50
            ),
        )
        ...
        self.logger.log(
            f"Risk Model: daily_loss={self.max_daily_loss:.1%}, scenario_loss={self.scenario_max_loss:.0%}",
            LogLevel.INFO,
        )
        ...
        self.Schedule.On(
            self.DateRules.EveryDay(), self.TimeRules.At(14, 55), self.CloseAllPositions
        )

        # Daily summary logging
        self.Schedule.On(
            self.DateRules.EveryDay(), self.TimeRules.At(15, 45), self.LogDailySummary
        )
    ...
    def InitializeUniverse(self):
        ...
        try:
            ...
            self.logger.log(
                f"Failed to add SPX: {e}, falling back to SPY", LogLevel.WARNING
            )
            # Fallback to SPY
    ...
    def OnData(self, data):
        ...
        if self.underlying_symbol in data.Bars:
            self.rv_calculator.update(
                self.underlying_symbol, data.Bars[self.underlying_symbol]
            )
    ...
    def _on_regime_change(
        self, old_regime: VolatilityRegime, new_regime: VolatilityRegime
    ):
    ...
    def CloseAllPositions(self):
        ...
        self.logger.log(
            "EOD FLATTEN: Closing all positions at 14:55 EST", LogLevel.INFO
        )
    ...
    def LogDailySummary(self):
        ...
        self.logger.log_daily_summary(
            tca_summary=tca_summary, risk_summary=risk_summary
        )
        ...
        self.logger.log_risk_snapshot(
            {
                ...
                "delta_by_bucket": {
                    k.value: v for k, v in snapshot.delta_by_bucket.items()
                },
                "vega_by_bucket": {
                    k.value: v for k, v in snapshot.vega_by_bucket.items()
                },
