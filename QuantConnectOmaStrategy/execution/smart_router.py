...
class SmartExecutionModel(ExecutionModel):
    ...
    def _execute_target(self, algorithm: QCAlgorithm, target: PortfolioTarget) -> None:
        ...
        try:
            ...
            if self.logger:
                self.logger.log(f"Execution error for {target.Symbol}: {e}", LogLevel.ERROR)
    ...
    def _calculate_limit_price(self, 
                               bid: float, 
                               ask: float, 
                               quantity: int,
                               mid: float) -> float:
    ...
    def _manage_working_orders(self, algorithm: QCAlgorithm) -> None:
        ...
        for order_id, tracker in self.working_orders.items():
            ...
            if order.Status in [OrderStatus.Filled, OrderStatus.Canceled, OrderStatus.Invalid]:
                orders_to_remove.append(order_id)
                continue
    ...
    def _replace_order(self, algorithm: QCAlgorithm, order_id: int, tracker: OrderTracker) -> None:
        """
        ...
        if theoretical_edge is not None:
            ...
            total_cost = spread_pct * 10000 * self.config.spread_cost_multiplier + self.config.impact_buffer_bps
            ...
            if theoretical_edge - total_cost < self.config.min_edge_after_costs_bps:
                ...
                if self.logger:
                    self.logger.log(
                        f"Edge gone - not replacing: {tracker.symbol}",
                        LogLevel.DEBUG
                    )
                return
    ...
    def _handle_fill(self, algorithm: QCAlgorithm, orderEvent) -> None:
        ...
        if tracker:
            ...
            self.execution_metrics['avg_slippage_bps'] = (avg_slippage * (n-1) + slippage_vs_mid_bps) / n
            
            avg_fill_time = self.execution_metrics['avg_fill_time']
            self.execution_metrics['avg_fill_time'] = (avg_fill_time * (n-1) + fill_time) / n
    ...
    def get_tca_summary(self) -> Dict:
        ...
        if summary['total_fills'] > 0:
            summary['avg_slippage_bps'] = summary['total_slippage_bps'] / summary['total_fills']
            summary['avg_spread_capture_bps'] = summary['total_spread_capture_bps'] / summary['total_fills']
            summary['limit_fill_pct'] = summary['limit_fills'] / summary['total_fills']
            summary['fill_rate'] = summary['total_fills'] / (summary['total_fills'] + summary['cancelled_orders']) if summary['total_fills'] + summary['cancelled_orders'] > 0 else 0
    ...
    def get_tca_by_symbol(self, symbol: Symbol = None) -> Dict:
        ...
        for sym_str, stats in by_symbol.items():
            if stats['fills'] > 0:
                ...
                stats['avg_spread_capture_bps'] = stats['total_spread_capture_bps'] / stats['fills']
